...
Func ConfigCreate($cfgFile)
   ...
   Else
	  ...
	  Dim $data[7][2] = [[1, ""], ["DeleteMails", 0], ["DestinationFolder", "D:\Outlook_backup"], ["BackupTime", "19:00:00"], ["LogEnable", "False"],["Outlook_Folder_Exclude_List", ""],["EmailID",""]]
...
Func ConfigRead()
   ...
   Else
	  ...
	  Global $mail_id = StringSplit(IniRead($cfgFile, "Section", "MailID", ""))
	  Global $delete_mails = StringSplit(IniRead($cfgFile, "Section", "DeleteMails", 0))
	  Global $logEnable = IniRead($cfgFile, "Section", "LogEnable", "False")
	  Global $ExcludeList = StringSplit(IniRead($cfgFile, "Section", "Outlook_Folder_Exclude_List", ""), ",")
	EndIf
   Return True
EndFunc
...
Func Service_Wait()
   SendAndLog("[Service_Wait]: Start")
      If IsArray($cfgtime) Then
	  If $cfgtime[0] = 3 Then
		 While Not (@HOUR = $cfgtime[1] And $cfgtime[2] = @MIN)
		 Sleep (1000*5)
		 WEnd
	  EndIf
   EndIf
...
Func Backup_mails($DestFolder, $ExcludeList, $delete_mails = 0)
...
Func Main()
   ...
   $out = Backup_mails($DestFolder, $ExcludeList, $delete_mails)
